...
const ReportPineline = ({ orders, loading, statusFilter }) => {
    ...
    if (activeTable === "installment" && !fetchedAllInstallments) {
      const fetchAll = async () => {
        ...
        try {
          ...
          if (!userId) return;
          // pass a very wide date range to get all time
  ...
  const filterOrders = (ordersList) => {
    return ordersList.filter((o) => {
      const catMatch =
        categoryFilter === "all" ||
        (o.products || []).some((p) => p.category === categoryFilter);
      const creatorMatch =
        creatorFilter === "all" || o.createdBy?._id === creatorFilter;
  ...
  const totalAmounts = useMemo(() => {
    return {
      pending: getStatusOrders("Pending").reduce(
        (s, o) => s + (o.amountTotal || 0),
        0,
      ),
      completed: getStatusOrders("Completed").reduce(
        (s, o) => s + (o.amountTotal || 0),
        0,
      ),
      cancelled: getStatusOrders("Cancelled").reduce(
        (s, o) => s + (o.amountTotal || 0),
        0,
      ),
      installment: getStatusOrders("Installment").reduce(
        (s, o) => s + (o.amountTotal || 0),
        0,
      ),
    };
  ...
  const addNoteToPipeline = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "https://www.system.crmkhitam.com/api/v1/pineline/add-note",
        {
  ...
  const handleExportExcel = () => {
    const data = currentOrders.map((o) => {
      ...
      const categories = [
        ...new Set((o.products || []).map((p) => p.category).filter(Boolean)),
      ].join(", ");

      const isBusinessPartner =
        o.isBusinessPartner === true || o.contact?.isBusinessPartner === true;

      return {
        "Mã đơn hàng": o.orderCode || "",
        "Người tạo": o.createdBy
          ? `${o.createdBy.lastname} ${o.createdBy.firstname}`
          : "Chưa phân công",
        ...
        "Sản phẩm": uniqueProducts
          .map(
            (p) =>
              `${p.name}${p.count > 1 ? ` x${p.count}` : ""} (${(p.price || 0).toLocaleString("vi-VN")} VND)`,
          )
          .join(" | "),
        "Danh mục": categories,
    ...
    const catLabel =
      categoryFilter === "all" ? "TatCa" : categoryFilter.replace(" ", "_");
    const creatorLabel =
      creatorFilter === "all"
        ? ""
        : `_${creatorList.find((c) => c.id === creatorFilter)?.name.replace(" ", "_") || ""}`;
  ...
  const getCategoryTag = (products) => {
    ...
    if (cats.includes("Academy") && cats.includes("Health Hub"))
      return <span style={styles.catMix}>Mixed</span>;
    if (cats[0] === "Academy")
      return <span style={styles.catAcademy}>Academy</span>;
    if (cats[0] === "Health Hub")
      return <span style={styles.catHub}>Health Hub</span>;
              ...
              <div style={styles.filterBar}>
                ...
                <div style={styles.filterGroup}>
                  ...
                  <div style={styles.creatorScroll}>
                    ...
                    {creatorList.map((c) => {
                      const orderCount = filterOrders(
                        orders.filter((o) => o.createdBy?._id === c.id),
                      ).length;
                      return (
                        <button
                          key={c.id}
              ...
              <div style={styles.heroTotal}>
                <div>
                  ...
                  <div style={styles.heroAmount}>
                    {getCurrentTotal().toLocaleString("vi-VN")} VND
                  </div>
                  <div style={styles.heroSub}>
                    {currentOrders.length} đơn hàng
                    {categoryFilter !== "all" && (
                      <span style={styles.heroPill}>{categoryFilter}</span>
                    )}
                    {creatorFilter !== "all" && (
                      <span style={styles.heroPill}>
                        {creatorList.find((c) => c.id === creatorFilter)?.name}
                      </span>
                    )}
                  </div>
                </div>
                ...
                {[
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...styles.tabBtn,
                        ...(activeTable === tab.key ? styles.tabBtnActive : {}),
                      }}
                    >
                      {tab.label}
                      <span
                      ...
                      <tbody>
                        {currentOrders.length === 0 ? (
                          ...
                          currentOrders.map((order) => {
                            const productMap = (order.products || []).reduce(
                              (acc, p) => {
                                const key = `${p.name}-${p.price}`;
                                acc[key] = acc[key] || { ...p, count: 0 };
                                acc[key].count++;
                            ...
                            const isBusinessPartner =
                              order.isBusinessPartner === true ||
                              order.contact?.isBusinessPartner === true;
                            ...
                            return (
                              <React.Fragment key={order._id}>
                                ...
                                >
                                  ...
                                  <td
                                    style={{ ...styles.td, maxWidth: "200px" }}
                                  >
                                  ...
                                  >
                                    {isBusinessPartner ? (
                                      <span style={styles.businessPartnerYes}>
                                        ĐỐI TÁC
                                      </span>
                                    ) : (
                                      <span style={styles.businessPartnerNo}>
                                        CÁ NHÂN
                                      </span>
                                    )}
                                  </td>
                                  ...
                                  >
                                    {(order.surcharge || 0).toLocaleString(
                                      "vi-VN",
                                    )}{" "}
                                    VND
                                  </td>
                                  ...
                                  >
                                    <div style={styles.amountCell}>
                                      {(order.amountTotal || 0).toLocaleString(
                                        "vi-VN",
                                      )}
                                      ...
                                      order.images.map((img, i) => (
                                        <button
                                          ...
                                          onClick={(e) => {
                                            e.stopPropagation();
                                            window.open(
                                              `https://www.system.crmkhitam.com${img.url}`,
                                              "_blank",
                                            );
                                          }}
                                          style={styles.imgBtn}
                                  ...
                                  >
                                    {order.updatedAt &&
                                    !isNaN(new Date(order.updatedAt)) ? (
                                      <span
                                        style={{
                                          color: "#fd7e14",
                                          fontWeight: "500",
              ...
              >
                <Modal.Header closeButton>
                  <Modal.Title style={{ fontSize: "1em", fontWeight: "600" }}>
                    Ghi chú — Đơn #{activeOrder?.orderCode}
                  </Modal.Title>
                </Modal.Header>
                <Modal.Body>
                  <Row className="mb-3">
                    <Col sm={6}>
                      ...
                      <div style={styles.modalInfo}>
                        <strong>Email:</strong>{" "}
                        {activeOrder?.contact?.email || "—"}
                      </div>
                      <div style={styles.modalInfo}>
                        <strong>SĐT:</strong>{" "}
                        {activeOrder?.contact?.phone || "—"}
                      </div>
                    </Col>
                    <Col sm={6}>
                      <div style={styles.modalInfo}>
                        <strong>Loại khách:</strong>
                        {activeOrder?.isBusinessPartner === true ||
                        activeOrder?.contact?.isBusinessPartner === true
                          ? "Đối tác / Doanh nghiệp"
                          : "Cá nhân"}
                      </div>
                      <div style={styles.modalInfo}>
                        <strong>Affiliate ID:</strong>{" "}
                        {activeOrder?.affiliate_id || "Không có"}
                      </div>
                      <div style={styles.modalInfo}>
                        <strong>Affiliate Name:</strong>{" "}
                        {activeOrder?.affiliate_name || "Không có"}
                      </div>
                    </Col>
                  </Row>
                  ...
                  <div style={styles.noteSection}>
                    ...
                    <div style={styles.noteBox}>
                      {activeOrder?.pipelineNotes || "Không có"}
                    </div>
                  </div>
