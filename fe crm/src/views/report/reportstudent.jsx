...
const BASE_URL = 'https://www.system.crmkhitam.com/api/v1/contact';
const API_PRODUCTS = 'https://www.system.crmkhitam.com/api/v1';
...
const S = {
  ...
  searchWrap: { display: 'flex', gap: '8px', alignItems: 'center', marginBottom: '20px' },
  searchInput: {
    height: '42px',
  ...
  resultTitle: { color: C.white, fontWeight: 700, fontSize: '15px', display: 'flex', alignItems: 'center', gap: '8px' },
  ...
  paginationWrap: { display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '4px', marginTop: '16px', flexWrap: 'wrap' },
  ...
  pageInfo: { textAlign: 'center', color: C.gray400, fontSize: '12px', marginTop: '8px' },
  ...
  modalClose: { background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: C.gray400, lineHeight: 1 },
  modalBody: { padding: '20px 24px' },
  modalFooter: { padding: '14px 24px', borderTop: `1px solid ${C.gray100}`, display: 'flex', justifyContent: 'flex-end', gap: '10px' },
  ...
  label: { fontWeight: 600, fontSize: '13.5px', marginBottom: '6px', display: 'block' },
...
/* ─── Helpers ──────────────────────────────────────────────────────────── */
const statusMapping = { Pending: 'Đang xử lý', Completed: 'Hoàn thành', Cancelled: 'Đã hủy', Installment: 'Trả góp' };
const paginate = (list, page, perPage) => list.slice((page - 1) * perPage, page * perPage);
const QUICK_RANGES = ['Hôm nay', '1 tuần qua', '1 tháng qua', '1 năm qua', 'Từ đầu'];
...
function PaginationBar({ total, current, perPage, onChange }) {
  ...
  const Btn = ({ label, page, disabled, active }) => (
    <button style={{ ...S.pageBtn(active, disabled) }} onClick={() => !disabled && onChange(page)} disabled={disabled}>
      {label}
    </button>
  );

  items.push(<Btn key="prev" label="‹" page={current - 1} disabled={current === 1} />);
  if (s > 1) {
  ...
  for (let i = s; i <= e; i++) items.push(<Btn key={i} label={i} page={i} active={i === current} />);
  if (e < totalPages) {
    if (e < totalPages - 1)
      items.push(
        <span
  ...
  items.push(<Btn key="next" label="›" page={current + 1} disabled={current === totalPages} />);

  return (
    <div>
      <div style={S.paginationWrap}>{items}</div>
      <div style={S.pageInfo}>
        Hiển thị {total === 0 ? 0 : (current - 1) * perPage + 1}–{Math.min(current * perPage, total)} / {total} bản ghi
      </div>
    </div>
  );
}
...
function EmailModal({ contact, onClose }) {
  ...
  const handleSend = () => {
    ...
    axios
      .post('https://www.system.crmkhitam.com/api/v1/emailmaketing/send', {
        to: to.trim(),
  ...
  return (
    <div style={S.modalOverlay} onClick={onClose}>
      <div style={{ ...S.modalBox, maxWidth: '600px' }} onClick={(e) => e.stopPropagation()}>
        ...
        >
          <span style={{ ...S.modalTitle, color: C.white, display: 'flex', alignItems: 'center', gap: '8px' }}>✉️ Soạn & Gửi Email</span>
          <button
            style={{
        ...
        <div style={S.modalBody}>
          ...
          <div style={S.emailFieldWrap}>
            <label style={{ ...S.label, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: '12px', color: C.gray400, fontWeight: 400 }}>{body.length} ký tự</span>
            </label>
        ...
        <div style={S.modalFooter}>
          ...
          <button style={{ ...S.btnPurple, ...(!canSend ? S.btnDisabled : {}) }} onClick={handleSend} disabled={!canSend}>
...
const ReportStudent = () => {
  ...
  const fetchBySearch = (keyword) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setTimeout(() => searchResultRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      })
      .catch((err) => {
        console.error(err);
        setSearchLoading(false);
  ...
  const fetchByProduct = (keyword) => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setTimeout(() => productResultRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 100);
      })
      .catch((err) => {
        console.error(err);
        setProductLoading(false);
    ...
    try {
      ...
      return (res.data || [])
        .filter((p) => {
          ...
          return code && !['n/a', 'null', 'undefined', 'nan', ''].includes(code.toLowerCase());
        })
        .map((p) => {
  ...
  const patchAssignContact = (contactId, userId) => axios.patch(`${BASE_URL}/assign/${contactId}`, { assignedTo: userId });
      ...
      .then(() => {
        ...
        const upd = (list) =>
          list.map((c) => {
            ...
            return { ...c, assignedTo: u ? { _id: u._id, firstname: u.firstname, lastname: u.lastname } : c.assignedTo };
          });
  ...
  const renderRows = (list, page, perPage) => {
    if (list.length === 0)
      return (
        <tr>
          <td colSpan={canAssign ? 9 : 8} style={{ padding: '40px', textAlign: 'center', color: C.gray400 }}>
            Không có dữ liệu
          </td>
        </tr>
      );
    return paginate(list, page, perPage).map((contact, index) => {
      const total = contact.pipelines?.reduce((sum, p) => sum + (p.amountTotal || 0), 0) || 0;
      const rowNum = (page - 1) * perPage + index + 1;
      const isHovered = hoveredRow === contact._id;
      return (
        <tr
          key={contact._id}
          style={{ background: isHovered ? C.gray50 : C.white, transition: 'background .12s' }}
          onMouseEnter={() => setHoveredRow(contact._id)}
          onMouseLeave={() => setHoveredRow(null)}
        >
          <td style={{ ...S.td, color: C.gray400, fontWeight: 500, width: '40px' }}>{rowNum}</td>
          <td style={{ ...S.td, fontWeight: 700, color: C.primary }}>{contact.profileCode || '—'}</td>
          <td style={S.td}>
            <span style={{ fontSize: '13px', color: C.gray700 }}>
              {contact.assignedTo ? `${contact.assignedTo.lastname || ''} ${contact.assignedTo.firstname || ''}`.trim() : '—'}
            </span>
          </td>
          <td style={S.td}>
            ...
            <div style={S.infoLine}>
              ...
              {contact.birthDate ? new Date(contact.birthDate).toLocaleDateString('vi-VN') : '—'}
            </div>
          </td>
          <td style={S.td}>
            <button style={S.btnDetail} onClick={() => handleViewProducts(contact.pipelines)}>
              📋 Xem chi tiết
            </button>
          </td>
          <td style={{ ...S.td, fontWeight: 700, color: C.gray800, whiteSpace: 'nowrap' }}>
            {total.toLocaleString('vi-VN')} <span style={{ color: C.gray400, fontWeight: 400 }}>₫</span>
          </td>
          <td style={{ ...S.td, color: C.gray600, whiteSpace: 'nowrap' }}>{new Date(contact.createdAt).toLocaleDateString('vi-VN')}</td>

          {/* ── Cột nút Gửi Email ── */}
          <td style={S.td}>
            {contact.email ? (
              <button style={S.btnEmail} onClick={() => handleOpenEmail(contact)} title={`Gửi email tới ${contact.email}`}>
                ✉️ Gửi email
              </button>
            ) : (
              <span style={{ fontSize: '12px', color: C.gray400 }}>Không có email</span>
            )}
          </td>

          {canAssign && (
            <td style={S.td}>
              <button style={S.btnAssign} onClick={() => handleOpenAssign(contact)}>
                🔄 Assign
              </button>
            </td>
          )}
    ...
    <div ref={refProp} style={S.resultCard(color)}>
      <div style={S.resultHeader(headerBg)}>
        <div style={S.resultTitle}>
          {title}
          {!isLoading && <span style={S.badge('rgba(255,255,255,.25)')}>{count} kết quả</span>}
        </div>
      ...
      <div style={{ padding: '20px' }}>
        {isLoading ? (
          ...
          <EmptyState icon={emptyIcon} text={`Không tìm thấy kết quả nào cho "${query}"`} hint={emptyHint} />
        ) : (
          <>
            <div style={{ overflowX: 'auto' }}>
              <table style={S.table}>
                <TableHead />
                <tbody>{renderRows(results, page, SEARCH_ITEMS_PER_PAGE)}</tbody>
              </table>
            </div>
            <PaginationBar total={results.length} current={page} perPage={SEARCH_ITEMS_PER_PAGE} onChange={setPage} />
          </>
        )}
      </div>
    </div>
  ...
  return (
    <div style={S.page}>
      {/* Tab bar */}
      <div style={S.tabBar}>
        <button style={S.tab(activeTab === 'customer', C.primary)} onClick={() => setActiveTab('customer')}>
          🔍 Tìm theo khách hàng
        </button>
        <button style={S.tab(activeTab === 'product', C.success)} onClick={() => setActiveTab('product')}>
          📦 Tìm theo sản phẩm
        </button>
      </div>
      ...
      {activeTab === 'customer' && (
        <div style={S.searchWrap}>
          ...
          <button
            style={{ ...S.btnPrimary, ...(!searchInput.trim() ? S.btnDisabled : {}) }}
      ...
      {activeTab === 'product' && (
        <div style={{ marginBottom: '20px' }}>
          <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
            ...
            <div style={{ flex: 1, maxWidth: '500px' }}>
              <AsyncSelect
                ...
                noOptionsMessage={({ inputValue }) => (inputValue.trim() ? 'Không tìm thấy sản phẩm' : 'Nhập tên sản phẩm để tìm kiếm')}
                ...
                styles={{
                  control: (base, state) => ({
                    ...
                    boxShadow: state.isFocused ? `0 0 0 3px ${C.success}22` : 'none',
                    transition: 'border-color .18s, box-shadow .18s',
                    '&:hover': { borderColor: C.success }
                  }),
                  placeholder: (base) => ({ ...base, color: C.gray400, fontSize: '14px' }),
                  menu: (base) => ({
                    ...base,
                    borderRadius: C.radius,
                  ...
                  option: (base, state) => ({
                    ...base,
                    fontSize: '13px',
                    background: state.isSelected ? C.success : state.isFocused ? C.successSoft : '#fff',
                    color: state.isSelected ? '#fff' : C.gray800,
                    cursor: 'pointer'
                  }),
                  singleValue: (base) => ({ ...base, fontSize: '14px', color: C.gray800 })
                }}
              />
            </div>
          ...
          {!selectedProduct && (
            <div style={{ fontSize: '12px', color: C.gray400, marginTop: '6px', paddingLeft: '2px' }}>
              💡 Gợi ý sẽ xuất hiện khi bạn gõ ít nhất 1 ký tự. Chọn sản phẩm để tìm khách hàng đã mua.
            </div>
          )}
          ...
          {selectedProduct && productQuery && (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginTop: '8px' }}>
              <span style={{ fontSize: '12px', color: C.gray600 }}>Đang xem khách hàng mua:</span>
              <span
                style={{
      ...
      {!hasSearched && !hasProductSearched && (
        <div style={S.filterBox}>
          ...
          <div style={S.filterRow}>
            {QUICK_RANGES.map((r) => (
              <button key={r} style={S.quickBtn(activeRange === r)} onClick={() => handleDateRange(r)}>
                {r}
              </button>
            ))}
      ...
      {!hasSearched && !hasProductSearched && (
        <div style={S.mainCard}>
          <div style={S.mainCardHeader}>
            ...
            {!loading && <span style={{ ...S.badge(C.gray100, C.gray600), marginLeft: '6px' }}>{contacts.length} bản ghi</span>}
          </div>
            ...
            {loading ? (
              ...
              <>
                <div style={{ overflowX: 'auto' }}>
                  <table style={S.table}>
                    <TableHead />
                    <tbody>{renderRows(contacts, currentPage, ITEMS_PER_PAGE)}</tbody>
                  </table>
                </div>
                <PaginationBar total={contacts.length} current={currentPage} perPage={ITEMS_PER_PAGE} onChange={setCurrentPage} />
              </>
            )}
          </div>
        </div>
      ...
      {showModal && (
        <div style={S.modalOverlay} onClick={() => setShowModal(false)}>
          <div style={{ ...S.modalBox, maxWidth: '760px' }} onClick={(e) => e.stopPropagation()}>
              ...
              {modalData.length === 0 ? (
                ...
                <div style={{ overflowX: 'auto' }}>
                  <table style={S.table}>
                    <thead>
                      <tr>
                        {['Sản Phẩm', 'Giá', 'Trạng Thái', 'Thanh Toán', 'Tổng Tiền'].map((h) => (
                          <th key={h} style={S.th}>
                            {h}
                          </th>
                        ))}
                      ...
                      {modalData.map((pipeline, i) => (
                        <tr key={i}>
                          ...
                          <td style={{ ...S.td, whiteSpace: 'nowrap' }}>
                            {pipeline.products?.map((p, j) => (
                              <div key={j} style={{ color: C.primary, fontWeight: 600 }}>
                                ...
                                background:
                                  pipeline.status === 'Completed'
                                    ...
                                    : pipeline.status === 'Cancelled'
                                      ? '#FEF2F2'
                                      : C.warningSoft,
                                color: pipeline.status === 'Completed' ? C.success : pipeline.status === 'Cancelled' ? C.danger : C.warning
                              }}
                            >
                              {statusMapping[pipeline.status] || pipeline.status}
                            </span>
                          </td>
                          <td style={S.td}>{pipeline.paymentType}</td>
                          <td style={{ ...S.td, fontWeight: 700, color: C.gray800, whiteSpace: 'nowrap' }}>{pipeline.totalAmount} ₫</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
      ...
      {showAssignModal && (
        <div style={S.modalOverlay} onClick={() => setShowAssignModal(false)}>
          <div style={{ ...S.modalBox, maxWidth: '480px' }} onClick={(e) => e.stopPropagation()}>
            <div style={S.modalHeader}>
              <span style={S.modalTitle}>🔄 Chuyển Khách Hàng</span>
              <button style={S.modalClose} onClick={() => setShowAssignModal(false)}>
                ×
              </button>
            </div>
            <div style={S.modalBody}>
              ...
              {assignSuccess && <div style={S.alertSuccess}>{assignSuccess}</div>}
              {assignError && <div style={S.alertDanger}>{assignError}</div>}
            </div>
            <div style={S.modalFooter}>
              <button style={S.btnSecondary} onClick={() => setShowAssignModal(false)}>
                Hủy
              </button>
              <button
                style={{ ...S.btnWarning, ...(!selectedUserId || assignLoading ? S.btnDisabled : {}) }}
